...
def label_reset() -> None:
    ...
    """
    message1.configure(
        text="Please  check  all  fields  properly  before  saving !", bg="#00aeff"
    )
    return
...
def RFID_scan() -> None:
    ...
    try:
        ...
        message1.configure(
            text="Please  enter  the  correct  port  where  device  is  connected!",
            bg="red",
        )
        return
    while 1:
            ...
            for lines in reader1:
                ...
                if (i > 1) and (i % 2 != 0):
                    ...
                    if str(lines[0]) == rfid_code:
                        message1.configure(
                            text="Card  is  already  registered,  please  take  a  new  card !!",
                            bg="red",
                        )
                    else:
...
def save_pass() -> None:
    ...
    if op == key:
        if newp == nnewp:
            ...
            c = conn.cursor()
            c.execute(
                """UPDATE Studentdetails SET NAME = ? WHERE NAME = ?""",
                  (newp, key)
            )
            conn.commit()
            c.close()
...
def change_pass() -> None:
    ...
    lbl4 = tk.Label(
        master, text="    Enter Old Password",
    ...
    lbl5 = tk.Label(
        master, text="   Enter New Password",
          bg="white", font=("times", 12, " bold ")
    )
    ...
    lbl6 = tk.Label(
        master, text="Confirm New Password",
          bg="white", font=("times", 12, " bold ")
    )
    ...
    warn = tk.Label(
        master,
        text="Check  new  password  properly  before  saving",
        fg="red",
        bg="white",
        width=38,
        font=("times", 12, " bold "),
        ...
        if v == 1:
            ...
            with open("Details\Details.csv", "r") as inputf:
                ...
                for lines in reader1:
                    if str(lines[2]) == Id:
                        message1.configure(
                            text="ID number is already alloted to someone", bg="red"
                        )
                        return
...
def update_profile() -> None:
    ...
    if exists:
        ...
        message1.configure(
            text="Details  are  missing,  please  check  Details  folder !", bg="red"
        )
...
def registration() -> None:
    ...
    frame1.place(relx=0.11, rely=0.11,
                  relwidth=0.78, relheight=0.45)

    frame2 = tk.Frame(window2, bg="#00aeff")
    frame2.place(relx=0.11, rely=0.57,
                  relwidth=0.78, relheight=0.40)
    ...
    lbl.place(x=50, y=34)
    global txt
    txt = tk.Entry(frame1, width=25, fg="black",
                    font=("times", 15, " bold "))
    txt.place(x=50, y=60)
    ...
    global txt2
    txt2 = tk.Entry(frame1, width=25, fg="black",
                     font=("times", 15, " bold "))
    ...
    global txt3
    txt3 = tk.Entry(frame1, width=25, fg="black",
                     font=("times", 15, " bold "))
    ...
    global txt4
    txt4 = tk.Entry(frame1, width=25, fg="black",
                     font=("times", 15, " bold "))
    ...
    global txt5
    txt5 = tk.Entry(frame1, width=25, fg="black",
                     font=("times", 15, " bold "))
    ...
    global txt6
    txt6 = tk.Entry(frame1, width=25, fg="black",
                     font=("times", 15, " bold "))
    ...
    global txt8
    txt8 = tk.Entry(frame1, width=57, fg="black",
                     font=("times", 15, " bold "))
    ...
    rfidmsg = tk.Label(
        frame2,
        text="Enter Port and click SCAN",
        width=20,
        fg="black",
        bg="white",
        font=("times", 15, " bold "),
    ...
    message1 = tk.Label(
        frame2,
        text="Please  check  all  fields  properly  before  saving !",
        width=50,
        height=1,
        fg="black",
        bg="#00aeff",
...
def make_icard() -> None:
    ...
    if exists1:
        ...
        message1.configure(
            text="Details  are  missing,  please  check  Details  folder !",
              bg="red"
        )
    with open("Details\Details.csv", "r") as csvFile2:
        reader1 = csv.reader(csvFile2)
            ...
            if str(lines[2]) == Id:
                ...
                while 1:
                    ...
                    if cv2.waitKey(1) & 0xFF == ord("s"):
                        ...
                        break
    if q == 0:
        message1.configure(text="ID  not  found  in records !",
                            bg="red")
    ...
    font = ImageFont.truetype("roboto\Roboto-Bold.ttf",
                               size=31)
    fontd = ImageFont.truetype("roboto\Roboto-Bold.ttf",
                                size=20)
    draw.text((30, 365), text=name.upper(),
               fill="orange", font=font)
    draw.text((60, 645), text="INSTITUTION ADDRESS",
               fill="white", font=font)
    draw.text((20, 20), text="INSTITUTION NAME & LOGO",
               fill="white", font=font)
    draw.text((30, 425), text="ID : " + id,
               fill="black", font=fontd)
    draw.text((30, 465), text="Contact no. : " + phone,
               fill="black", font=fontd)
    draw.text((30, 505), text="Blood Group : " + blood,
               fill="black", font=fontd)
    draw.text((30, 545), text="D.O.B. : " + dob,
               fill="black", font=fontd)
    draw.text((30, 585), text=mail, fill="red",
               font=fontd)
...
def start_attendance(aport) -> None:
    ...
    try:
        ...
        mess._show(
            title="Port Error",
            message="Please  enter  the  correct  port  where  device  is  connected!",
        )
        return
...
frame1.place(relx=0.12, rely=0.17,
              relwidth=0.76, relheight=0.80)

message3 = tk.Label(
    window,
    text="RFID Based Attendance System",
    fg="white",
    bg="#262523",
    width=55,
    height=1,
...
clock = tk.Label(
    frame3, fg="orange", bg="#262523", width=55, height=1,
      font=("times", 22, " bold ")
)
...
txt7 = tk.Entry(frame1, width=18, fg="black",
                 font=("times", 15, " bold "))
txt7.place(x=345, y=57)

message = tk.Label(
    frame1,
    text="ENTER PORT OF DEVICE",
    bg="#00aeff",
    fg="black",
    width=22,
    height=1,
...
register_update = tk.Button(
    frame1,
    text="Register  or  Update  Profile",
